...
class LidarScanner:
    ...
    def apply_angular_noise(self, ray_lidar: np.ndarray, angle_noise_std: float) -> np.ndarray:
        ...
        # 添加高斯噪声到角度
        theta += np.random.normal(0, angle_noise_std, size=theta.shape)
        phi += np.random.normal(0, angle_noise_std, size=phi.shape)
    ...
    def apply_noise(self, point_cloud: np.ndarray, rays_world: np.ndarray, mesh: trimesh.Trimesh, angle_noise_std,
        ...
        distance_noise = np.random.normal(0, distance_noise_std, size=distances.shape)
